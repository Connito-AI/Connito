...
class TestBaseIsolation:
    ...
    def test_a_cold_start_round_has_no_shard(self) -> None:
        """No baseline published yet: the pretrained backbone is the base."""
        config = _fake_validator_config()
        rnd = _freeze_round(
            config=config, metagraph=_make_metagraph({"hk_a": 0.5}),
            assignment={"vhk": ["hk_a"]}, base_shard=None,
        )
        assert rnd.base_shard is None
