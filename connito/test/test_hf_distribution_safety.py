...
def test_fetch_model_skips_when_chain_checkpoint_lacks_hf_coords(tmp_path, monkeypatch):
    """A chain checkpoint without HF coords is unreachable now that the
    validator HTTP fallback is gone. The miner skips it instead of falling
    through to a deleted endpoint.
    """
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: (_ for _ in ()).throw(AssertionError("HF should not be called when coords are missing")),
    )
    ...
    # No HF coords + no HTTP fallback → no checkpoint downloaded; the miner
    # treats the round as unavailable and the function returns None (the
    # implicit return that follows the retry loop).
...
def test_hydrate_miner_submissions_from_hf_writes_assigned_miners_only(tmp_path, monkeypatch):
    ...
    # Miner with no HF coords is skipped — HF is now the only submission
    # transport, so they will be missing for this round.
...
def test_hydrate_miner_submissions_skips_when_local_file_already_present(tmp_path, monkeypatch):
    submission_dir = tmp_path / "miner_submission"
    submission_dir.mkdir()
    # Simulate a submission file that already landed (e.g. from a prior
    # hydration pass or the background download worker).
    (submission_dir / "hotkey_miner-assigned_block_500.pt").write_bytes(b"existing")
