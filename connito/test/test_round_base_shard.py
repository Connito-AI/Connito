...
(`shared/model.freeze_parameters`), so the active group's shard plus a
pretrained backbone reproduces exactly the base a round's miners trained
from. These pin that the worker really rebuilds from the file.
"""
...
def _round(base_shard: Path | None) -> SimpleNamespace:
...
def test_the_base_comes_from_the_shard_not_the_resident_weights(tmp_path, stub_eval):
    ...
    shard = tmp_path / "round_99.safetensors"
...
def test_a_cold_start_round_leaves_the_backbone_alone(tmp_path, stub_eval):
    """No baseline published yet: the pretrained backbone is the base."""
    worker = _worker(_Tiny(val=0.1))

    asyncio.run(worker._load_round_base(_round(None)))

    torch.testing.assert_close(
        worker._eval_base_model.lin.weight, torch.full((2, 4), 0.1)
    )
