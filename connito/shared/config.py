...
class ModelCfg(BaseConfig):
    ...
    device: str = "cuda"
    # Runtime weight-only int8 quantization. Weights are always *loaded* and
    # *saved* at `precision`; "int8" only changes how they are held in memory
    # between those points. See connito/shared/modeling/quantization.py.
    #
    ...
    #                silently skip int8 buffers). Covers the routed experts, so
    ...
    #
    # CONSENSUS: this changes `val_loss`. A validator running "int8" produces
    ...
    quantization: Literal["off", "int8"] = "off"
