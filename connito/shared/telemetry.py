...
# Per-source validation loss telemetry. The eval pool is a 50/50 mix of
# allenai/c4 (prose) and nvidia/Nemotron-CC-Math-v1 (math); a single
# aggregate `miner_val_loss` hides whether we are being beaten on prose
# or on math, which is the actionable signal for choosing where to invest
# data-quality work. These gauges are emitted from `evaluate_model` only
# when the caller passes `source_attribution` (Unit 8 mini-eval / Unit 15
# best-ckpt-selector); legacy callers continue to see the aggregate-only
# return schema.
MINER_VAL_LOSS_BY_SOURCE = Gauge(
    "miner_val_loss_by_source",
    "Validation loss split by data source",
    labelnames=("expert_group", "rank", "source"),
)
MINER_VAL_LOSS_BATCH_HISTOGRAM = Histogram(
    "miner_val_loss_batch",
    "Per-batch validation loss distribution",
    labelnames=("expert_group", "rank", "source"),
    buckets=(0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0, 7.0, 10.0, 20.0, float("inf")),
)
MINER_VAL_SCORED_BATCHES_BY_SOURCE = Gauge(
    "miner_val_scored_batches_by_source",
    "Number of non-NaN batches per source",
    labelnames=("expert_group", "rank", "source"),
)
