...
def scheduler_service(
    ...
    Submission is HF-only: the miner uploads to Hugging Face during the
    commit phase and the chain commit advertises the (repo, revision); the
    validator then pulls the bytes from HF. There is no separate submission
    job.
    """
    while True:
        # --------- DOWNLOAD SCHEDULING ---------
...
def _prepare_checkpoint_for_commit(
    ...
    """Pick the latest local checkpoint, sign it, and publish the path to shared state."""
...
def _upload_checkpoint_to_hf_safe(
    ...
    a failure here means the miner is missing for this round and will be
    penalized by the validator's missed-submission pass.
    """
    try:
...
def commit_worker(
    ...
    while True:
        ...
        try:
            ...
            # by the time we write miner_commit_2. HF is the only submission
            # transport; on failure (None, None) the chain commit goes out
            # without r/rv and the validator records this miner as missing
            # for the round.
