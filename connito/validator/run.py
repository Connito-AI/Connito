...
def run(rank: int, world_size: int, config: ValidatorConfig, pkg_version: str = "") -> None:
    ...
    if pkg_version == "v0.1.22":
        # One-time wipe: drop any prior aggregator state on disk so the v0.1.22
        # rollout starts every validator with a clean score history. Subsequent
        # restarts on v0.1.22 fall through the `score_path.exists()` branch and
        # load whatever this version has persisted.
        logger.info("Clearing historic score_aggregator for v0.1.22", pkg_version=pkg_version)
