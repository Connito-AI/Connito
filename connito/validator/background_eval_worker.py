...
class BackgroundEvalWorker(threading.Thread):
    ...
    async def _load_round_base(self, round_obj) -> None:
        ...
        The shard carries only the active expert group; every other
        parameter is frozen for the life of the subnet, so `strict=False`
        with the backbone left alone reconstructs the exact base the round's
        miners trained from. `base_shard=None` is a cold start — the
        pretrained backbone is the base.
        ...
        def _load() -> None:
            shard = getattr(round_obj, "base_shard", None)
            if shard is None:
                return
