...
> **Base note.** This work was first derived against `origin/staging` @ `6d7b58e`
> and then retargeted to `staging_v2`, which is a strict ancestor of `staging` —
> 39 commits behind it, and byte-identical to `master`. The findings were **not
> replayed as a diff**; the whole analysis was re-run against this tree, because
> "dead on one tree" does not imply "dead on another". Every §1 finding
> reproduced here unchanged. The differences the re-run surfaced are recorded in
> §7.
...
(349 rather than 471 because `staging_v2` has nine fewer test modules — see §7.)
...
## 7. Retargeting from `staging` to `staging_v2`

`staging_v2` (`4db4d8d`) is not a newer branch. It is a strict ancestor of
`origin/staging` (`6d7b58e`): 39 commits behind, nothing of its own, and
identical to `master`. It lacks fp8 quantization, the merge-loss dedup filter,
validator observer mode, selectable expert topology and the full-topology
graft-in-place eval base — along with 20 files that exist only on `staging`
(`validator/dedup.py`, `validator/full_topology_eval.py`,
`shared/modeling/quantization.py`, `tools/quantization/*` and nine test
modules).

A `git rebase --onto` was attempted first and rejected as the wrong method. It
conflicted in two files, and one conflict showed exactly why replaying is
unsafe here: in `shared/model.py`, the `grad_hook` this cleanup deletes sits at
the same location as `_download_expert_shards`, a function that exists only on
`staging`. Resolving that conflict mechanically would have imported
`staging`-only code into a `staging_v2`-based branch. The rebase was aborted
and the analysis re-run from scratch against `staging_v2`.

**Result: every §1 finding reproduces on `staging_v2` unchanged.** The same two
files are unreachable from every entry point, tool and test; `MODEL_BACKEND` is
the same single-branch hardcoded constant; `create_expert_groups`,
`split_into_groups`, `sync_weights`, `sync_expert_weights`, `download_model`,
`SignedMessage` and the rest have the same zero AST references; `name_selected`
still has exactly one referent in the `select_tensors` this removes.

Two differences the re-run surfaced, both of which a diff replay would have
gotten wrong:

| Difference | On `staging` | On `staging_v2` | Handling |
|---|---|---|---|
| `shared/config.py` `from typing import … Literal` | Used (3 references) — **must stay** | Unused (import only) — dead | Removed here, correctly *not* removed on the `staging` branch |
| `shared/model.py` import block | Imports `HFFileMissingError` for the `staging`-only `_download_expert_shards` | No such import or function | Resolved natively; no `staging` code carried over |

Deferred items (§2) and confirmed-live items (§3) are unchanged on this base,
with one arithmetic note: the unused-import count is 43 rather than 42, because
of the `Literal` in the table above.
